...
const steps = [
  ...
  { name: 'Renewable Plant', icon: Factory, color: 'text-emerald-400' },
];
...
export default function CarbonLoop({ requests, socket }) {
  ...
  return (
    <div className="space-y-8">
          ...
          {steps.map((step, idx) => (
            <React.Fragment key={idx}>
              <div className="flex flex-col items-center text-center w-24">
                <div className={`p-3 rounded-full bg-slate-900 border border-slate-700 mb-2 ${step.color}`}>
            ...
            <tbody className="divide-y divide-slate-700">
              {requests && requests.length > 0 ? requests.map(req => (
                <tr key={req.id} className="hover:bg-slate-700/30">
                  ...
                  <td className="px-6 py-4 text-emerald-400">{parseFloat(req.capturedAmount || 0).toFixed(1)} kg</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      ...
                      req.status === 'Sent to Renewable Plant' ? 'bg-emerald-500/20 text-emerald-400' :
                  ...
                  <td className="px-6 py-4">
                    {req.status !== 'Sent to Renewable Plant' && (
                      <button onClick={() => updateStatus(req.id, getNextStatus(req.status))} className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded transition-colors">
                        Advance Status
                      </button>
                    )}
                    {req.status === 'Sent to Renewable Plant' && (
                      <span className="text-xs text-emerald-400 flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Completed</span>
                    )}
                  </td>
                </tr>
              )) : (
                <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">No active pickup requests.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-emerald-900/30 border border-emerald-500/20 rounded-xl p-4 flex items-start gap-3">
        <Factory className="w-5 h-5 text-emerald-400 flex-shrink-0 mt-0.5" />
        <div>
          <h4 className="font-semibold text-emerald-300 text-sm">Renewable Conversion Note</h4>
          <p className="text-xs text-slate-400 mt-1">Collected CO₂ is transported to the renewable conversion plant where it is combined with renewable hydrogen to produce synthetic/renewable fuel, completing the closed carbon-management ecosystem. The roadside device releases treated air after pollutant reduction.</p>
        </div>
      </div>
    </div>
  );
